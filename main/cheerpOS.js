...
function cheerpjFSInit(appUrlPrefix, jarJsOverridePath, loaderPath, useIframe) {
    ...
    if (useIframe) ipfCreateIFrame(loaderPath)
}
...
function cheerpOSNetInit(path_, tailscaleLoginUrlCb, tailscaleAuthKey, tailscaleControlUrl, tailscaleDnsIp, tailscaleNetmapUpdateCb, cb) {
    ...
    import(path_).then((net) => {
        ...
        net.autoConf({ loginUrlCb: loginCb, stateUpdateCb: stateUpdate, netmapUpdateCb: tailscaleNetmapUpdateCb, authKey: tailscaleAuthKey, controlUrl: controlUrl, dnsIp: tailscaleDnsIp }).then((netExports) => {
...
function folderGetCached(fileName) {
    ...
    if (!c.hasOwnProperty(fileName)) return null
...
function folderSetCached(fileName, fileData) {
    var c = this.fileCache
    fileData.cacheRefCount = (fileData.cacheRefCount + 1) | 0
    if (c.hasOwnProperty(fileName)) this.decRefCached(fileName, c[fileName])
...
function cheerpOSSafeContentLength(contentRangeStr) {
    if (contentRangeStr == null) return -1
    // Partial content with a Content-Range header
    if (!contentRangeStr.startsWith("bytes ")) return -1
...
function ddlOnLoad(resp, downloader) {
    if (resp.status == 206) {
        ...
        downloader.responseURL = resp.url
        if (resp.status == 231) downloader.responseURL += "/"
        downloader.onEnd(null)
        return
    ...
    reader.read().then(function (result) {
        if (!result.done) {
            ...
            if (downloader.chunkSize == 0) {
                ...
                while (dataLen > 0) {
                    if (chunkAvail > dataLen) chunkAvail = dataLen
            ...
            if (downloader.chunkSize == 0) {
                ...
                if (downloader.chunkOffset != 0) downloader.onData(downloader.chunk.subarray(0, downloader.chunkOffset))
                downloader.onEnd(null)
            }
        }
    })
...
function ddlSend() {
    ...
    var headers = {}
    var method = "GET"
    if (downloader.rangeHeader) headers["Range"] = downloader.rangeHeader
    else if (downloader.metaDataOnly) headers["Range"] = "bytes=0-0"
...
function webStatAsyncImpl(mp, path, fileRef, onData, cb) {
    ...
    dl.onEnd = function (allChunks) {
        ...
        if (this.lastModified) fileRef.lastModified = (Date.parse(this.lastModified) / 1000) | 0
        else fileRef.lastModified = 0
        return cb()
    }
    dl.send()
}
...
function webMakeFileData(mp, path, mode, uid, gid, cb) {
    ...
    webStatAsyncImpl(mp, path, fileRef, onData, function () {
        if (fileRef.permType === 0) return cb(null)
        ...
        for (var i = chunks.length; i < numChunks; i++) chunks[i] = null
        return cb(fileData)
    })
}
...
function webReadAsync(fileData, fileOffset, buf, off, len, flags, cb) {
    ...
    if (mp.resTrace) {
        ...
        for (; i < ranges.length; i += 2) {
            if (ranges[i] < startOffset) continue
            break
        }
    ...
    while (curChunk <= endChunk) {
        ...
        if (fileData.chunks[curChunk]) {
            fileData.chunks[curChunk].push(function () {
                fileRef.pendingLoads--
                if (fileRef.pendingLoads !== 0) return
...
function webReadAsyncEnd(fileData, fileOffset, buf, off, len, flags, fileRef, curChunk, endChunk, cb) {
    ...
    while (i < len) {
        ...
        if (len - i < chunkSize - curOffset) chunkSize = curOffset + (len - i)
    ...
    dl.onEnd = function (allChunks) {
        if (allChunks != null) {
            ...
            for (var i = 0; i < allChunks.length; i++) {
                ...
                for (var j = 0; j < chunkLen; j++) res += String.fromCharCode(chunk[j])
            }
            var files = res.split("\n")
            var filesLen = files.length
            for (var i = 0; i < filesLen; i++) {
                if (files[i].length) fileRef.push(files[i])
            }
        }
        cb()
    }
    ...
    idbEnsureDBConnection(mp, function () {
        ...
        function gotRes(fileRef, res) {
            if (!res) {
                ...
                fileRef.uid = res.uid
                fileRef.gid = res.gid
                if (res.hasOwnProperty("permType")) fileRef.permType = res.permType
                else if (res.type == "dir") fileRef.permType = CheerpJFileData.S_IFDIR | 0777
                else if (res.type == "s") fileRef.permType = CheerpJFileData.S_IFSOCK | 0666
                else {
                    assert(res.type == "file")
                    fileRef.permType = CheerpJFileData.S_IFREG | 0666
                }
                if (res.type == "file") fileRef.fileLength = res.contents == null ? 0 : res.contents.length
                else if (res.type == "special") fileRef.contents = res.contents
            }
            return cb()
        }
...
function idbListAsync(mp, path, fileRef, cb) {
    idbEnsureDBConnection(mp, function () {
        ...
        req.onsuccess = function (e) {
            ...
            if (res && res.type == "dir") {
                var c = res.contents
                for (var i = 0; i < c.length; i++) fileRef.push(c[i].substr(1))
            }
...
function idbCreateDirAsync(mp, path, fileRef, mode, uid, gid, cb) {
    idbEnsureDBConnection(mp, function () {
        ...
        function gotParent(res, asyncThread) {
            if (!res) fileRef.exists = 0
            else fileRef.exists = res.type == "dir" ? 5 : 3
            if (fileRef.exists !== 5) {
...
function idbDecRefIndirect(store, fileRef) {
    ...
    if (fileRef.refCount == 0) store.delete(fileRef.inodeId)
    else store.put(fileRef, fileRef.inodeId)
}
...
function idbResolve(mp, path, fileRef, cb) {
    ...
    if (cachedDE) {
        ...
        var req = store.get(parentPath)
        assert(req.readyState != "done")
        if (req.readyState == "done") doAsync = gotParent(null, null, store)
        else {
            req.onerror = idbTrap
            req.onsuccess = function (e) {
                this.onerror = null
                this.onsuccess = null
                gotParent(this.result, cb, this.source)
            }
            doAsync = true
            assert(req.readyState != "done")
        }
                ...
                if (mode == "r" || ((mode == "r+" || mode == "w") && fileExists)) {
                    if (fileExists) {
                        if (res.type == "file") {
                            ...
                            if (mode == "w") fileRef.fileData.dirty = 1
                    ...
                    if (fileExists) fileRef.fileData = null
                    else {
                        // Add this new object to the parent Path
                        var childPath = path.substring(parentPath.length)
                        assert(childPath[0] == "/")
...
function idbCommitFileData(fileData, cb) {
    ...
    if ((fileData.permType & CheerpJFileData.S_IFMT) != CheerpJFileData.S_IFREG) {
        ...
        for (var i = 0; i < fileData.length; ) {
            var c = fileData.chunks[curChunk]
            if (fileData.length - i < chunkSize) c = c.subarray(0, fileData.length - i)
            contents.set(c, i)
            curChunk++
            i += c.length
        }
        ...
        idbResolve(mp, path, fileRef, function () {
            if (fileRef.res == null) {
                ...
                if (res.type == "dir") {
                    ...
                    if (res.contents.length > 0) return cb(false)
                }
                var filePath = path.substr(parentPath.length)
                ...
                store.put(parentRes, parentPath)
                store.delete(path)
                if (fileRef.res.hasOwnProperty("refCount")) idbDecRefIndirect(store, fileRef.res)
            }
        })
    })
}
...
function strReadAsync(fileData, fileOffset, buf, off, len, flags, cb) {
    if (len > fileData.data.length - fileOffset) len = fileData.data.length - fileOffset
    // EOF
    if (len == 0) {
        return cb(0)
    }
    for (var i = 0; i < len; i++) buf[(off + i) >>> 0] = fileData.data[(fileOffset + i) | 0]
    return cb(len)
}
...
function cheerpjDefaultConsoleWrite(param, fileOffset, buf, off, len, cb) {
    ...
    if (c) c.textContent += strBytes
    else console.log(strBytes)
    return cb(len)
}
...
function cheerpjNormalizePath(path) {
    ...
    for (var i = 0; i < parts.length; i++) {
        if (parts[i] == "." || parts[i] == "") continue
            ...
            if (newParts.length) newParts.pop()
            else partsUnderflow++
...
function cheerpOSUnlinkMain(mount, path_, cb) {
    ...
    if (!mount.mountOps.unlinkAsync) return cb(-1)
...
function cjGraceTimeExpire() {
    ...
    for (var f in mount.fileCache) {
        if (fileData.inodeId == mount.fileCache[f]) mount.clearCached(f)
    }
        ...
        if (fileData.refCount === 0 && fileData.parent != null) {
            if (fileData.dirty) {
                for (var p in cosFileWatchPrefixes) {
                    ...
                    if (path === p || path.startsWith(p + "/")) cosFileWatchPrefixes[p](path)
                }
            }
            ...
            if (prevIdx >= 0) cjGraceTimeQueue[prevIdx] = null
...
function httpSocketWriteAsync(fileData, fileOffset, buf, off, len, cb) {
    ...
    if (fileData.currRequest.headersNewLines >= 1) {
        ...
        if ((fileData.currRequest.method === "POST" && fileData.currRequest.dataIdx < fileData.currRequest.headers.get("Content-Length")) || (fileData.currRequest.method !== "POST" && fileData.currRequest.headersNewLines < 1)) {
            return cb(cnt)
        }
...
function cheerpOSPoll(fds, fd, timeout, cb) {
    ...
    var fileDesc = fds[fd]
    if (fileDesc == null) return cb(-1)
    if (fileDesc.fileData.chunks.length > 0) return cb(1)
...
function cheerpOSReadAvailable(fds, fd, cb) {
    ...
    for (var i = 0; i < data.chunks.length; i++) {
        if (data.chunks[i]) tot += data.chunks[i].length
    }
    if (data.currData) {
...
function cheerpOSSocketListen(fds, fd, backlog, cb) {
    ...
    var fileData = fileDesc.fileData
    return cb(
        fileData.data.listen((socket, addr, port, err) => {
            assert(err == 0)
...
function cheerpOSSocketRecv(fds, fd, cb) {
    ...
    if (fd < 0) return cb({ data: null, addr: 0, port: 0 })
    var fileDesc = fds[fd]
    if (fileDesc == null) return cb({ data: null, addr: 0, port: 0 })
    var fileData = fileDesc.fileData
    var doRead = () => {
        var first = fileData.chunks.shift()
        return cb(first)
...
function cheerpOSResolveHost(hostName, cb) {
    if (hostName == "cheerpjhost") hostName = "127.0.0.1"
    if (!cjEnableTailscale) return cb((192 << 24) | (168 << 16) | (1 << 8) | 1)
...
function cheerpOSWatchFiles(prefix, cb) {
    ...
    if (cb == null) delete cosFileWatchPrefixes[prefix]
    else cosFileWatchPrefixes[prefix] = cb
}
...
// export {
//     cheerpjFSMounts,
//     cheerpjRuntimeResources,
//     cosRuntimeResourcesRanges,
//     cosFileWatchPrefixes,
//     RootOps,
//     RootInodeOps,
//     WebOps,
//     WebInodeOps,
...
//     webGetInode,
//     DirectDownloader,
//     cheerpOSSafeContentLength,
//     ddlOnLoad,
//     ddlOnData,
//     ddlOnError,
//     ddlSend,
//     IFrameProxyDownloader,
//     ipfMessage,
//     ipfCreateIFrame,
//     ipfSend,
//     webStatAsyncImpl,
...
//     cheerpjDefaultConsoleWrite,
//     cheerpOSAddStringFile,
//     cheerpOSAddStringFileMain,
//     cheerpOSRemoveStringFile,
//     cheerpjAddStringFile,
//     cheerpjRemoveStringFile,
//     cheerpjGetFSMountForPath,
//     cheerpjNormalizePath,
//     cheerpOSStat,
//     cheerpOSStatMain,
//     cheerpOSListFiles,
//     cheerpOSListFilesMain,
//     cheerpOSCreateDir,
//     cheerpOSCreateDirMain,
//     cheerpOSInitFds,
//     cheerpOSCopyFds,
//     cheerpOSAllocateFd,
//     cheerpOSDupFd,
//     cheerpjWakeOpenThreads,
//     cheerpOSOpen,
//     cheerpOSOpenMain,
//     cheerpOSRead,
//     cheerpOSWrite,
...
//     socketReadAsync,
//     socketReadLoop,
//     cheerpNetReadFetchOS,
//     httpSocketWriteAsync,
//     socketWriteAsync,
//     udpSocketReadAsync,
//     udpSocketWriteAsync,
//     socketClose,
//     cheerpOSSocketOpenInternal,
//     cheerpOSSocketOpen,
//     cheerpOSSocketShutdown,
//     cheerpOSPoll,
//     cheerpOSReadAvailable,
//     cheerpOSSocketConnect,
//     cheerpOSSocketListen,
//     cheerpOSSocketAccept,
//     cheerpOSSocketBind,
//     cheerpOSSocketSendTo,
//     cheerpOSSocketRecv,
//     cheerpOSResolveHost,
//     cheerpOSGetFileBlobFd,
//     cheerpOSGetFileBlob,
//     cheerpOSWatchFiles,
//     cheerpOSSeek,
//     cheerpOSFStat,
